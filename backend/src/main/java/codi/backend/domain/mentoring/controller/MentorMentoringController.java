...
public class MentorMentoringController {
    ...
    // 받은 신청 내역 전체 조회 GET
    @ApiOperation(value = "Mentoring 받은 신청 내역 전체 조회", notes = "신청된 모든 멘토링 정보를 조회한다.")
    @GetMapping("/{mentor-id}/application")
    public ResponseEntity getAllMentoringApplication(@PathVariable("mentor-id") Long mentorId, @RequestParam String order, Pageable pageable) {
    ...
    public ResponseEntity getDailyMentoring(@PathVariable("mentor-id") Long mentorId, @Valid MentoringDto.DailyRequest mentorDailyRequest) {
        return new ResponseEntity<>(new SingleResponseDto<>(mentoringService.findDailyMentoringsOfMentor(mentorId, mentorDailyRequest.getDate())), HttpStatus.OK);
    }
    ...
    public ResponseEntity getMonthlyMentoring(@PathVariable("mentor-id") Long mentorId, @Valid MentoringDto.MonthlyRequest monthlyRequest) {
        return new ResponseEntity<>(new SingleResponseDto<>(mentoringService.findMonthlyMentoringsOfMentor(mentorId, monthlyRequest.getMonth())), HttpStatus.OK);
    }
}
