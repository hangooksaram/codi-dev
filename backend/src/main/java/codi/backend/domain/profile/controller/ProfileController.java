...
public class ProfileController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "프로필 생성", content =
    ...
    public ResponseEntity createProfile(@AuthenticationPrincipal CustomUserDetails principal,
        ...
        ProfileDto.ProfileResponse profileResponse = profileMapper.profileToProfileResponse(profile);
        ...
        return new ResponseEntity<>(profileResponse, headers, HttpStatus.CREATED);
    }

    // 프로필 정보 수정
    @Operation(summary = "프로필 정보 수정", description = "프로필 이미지, 직무, 연차, 학력, 장애구분, 중증도, 장애기간, 소개를 선택해서 수정할 수 있다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "프로필 정보 수정", content =
                    { @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ProfileDto.ProfileResponse.class))})
    })
    @PatchMapping
    public ResponseEntity updateProfile(@AuthenticationPrincipal CustomUserDetails principal,
                                        ...
                                        @RequestPart(value = "file", required = false) MultipartFile file) {
        log.info("profile id: {}", principal.getProfileId());
    ...
    @Operation(summary = "프로필 이미지 삭제(수정 필요)", description = "memberId에 해당하는 사용자의 프로필 이미지를 삭제한다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "프로필 이미지 삭제", content =
                    { @Content(mediaType = "application/json")})
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "프로필 페이지 조회", content =
                    { @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ProfileDto.ProfileResponse.class))})
    })
